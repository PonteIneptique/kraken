...
class SaveModelCallback(Callback):
    def on_validation_end(self, trainer: "KrakenTrainer", pl_module: "pl.LightningModule") -> None:
        if not trainer.sanity_checking:
            # fill one_channel_mode after 1 iteration over training data set
            if trainer.current_epoch == 0 and trainer.model.nn.model_type == 'recognition':
